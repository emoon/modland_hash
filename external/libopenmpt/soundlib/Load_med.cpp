...
static std::pair<CHANNELINDEX, SEQUENCEINDEX> MEDScanNumChannels(FileReader &file, const uint8 version)
{
	...
	for (SEQUENCEINDEX song = 0; song < numSongs; song++)
	{
		...
		{
			if (!file.Seek(fileHeader.blockArrOffset + pat * 4u) || !file.Seek(file.ReadUint32BE()))
			{
				continue;
			}
...
{
	if (std::memcmp(fileHeader.mmd, "MMD", 3) || fileHeader.version < '0' || fileHeader.version > '3' || fileHeader.songOffset < sizeof(MMD0FileHeader) || fileHeader.songOffset > uint32_max - 63 * sizeof(MMD0Sample) - sizeof(MMDSong) || fileHeader.blockArrOffset < sizeof(MMD0FileHeader) || (fileHeader.sampleArrOffset > 0 && fileHeader.sampleArrOffset < sizeof(MMD0FileHeader)) || fileHeader.expDataOffset > uint32_max - sizeof(MMD0Exp))
	{
		return false;
	}
	return true;
...
static uint64 GetHeaderMinimumAdditionalSize(const MMD0FileHeader &fileHeader)
{
	return std::max<uint64>({fileHeader.songOffset + 63 * sizeof(MMD0Sample) + sizeof(MMDSong),
							 ...
							 fileHeader.expDataOffset + sizeof(MMD0Exp)}) -
		   sizeof(MMD0FileHeader);
}
...
bool CSoundFile::ReadMED(FileReader &file, ModLoadingFlags loadFlags)
{
			...
			if (ins.nMixPlug > 0)
			{
				...
				if (mixPlug.Info.dwPluginId2 == PLUGMAGIC('M', 'M', 'I', 'D'))
				{
					...
					for (PLUGINDEX p = 0; p < plug; p++)
					{
						...
						if (otherPlug.Info.dwPluginId1 == mixPlug.Info.dwPluginId1 && otherPlug.Info.dwPluginId2 == mixPlug.Info.dwPluginId2 && otherPlug.pluginData == mixPlug.pluginData)
						{
							ins.nMixPlug = p + 1;
							mixPlug = {};
							break;
	...
	for (SEQUENCEINDEX song = 0; song < numSongs; song++)
	{
		...
		else
		{
			...
			if (!file.Seek(header.sectionTableOffset) || !file.CanRead(songHeader.songLength * 2) || !file.ReadVector(sections, songHeader.songLength))
				continue;
		...
		if (version >= 2 && expData.trackInfoOffset)
		{
			for (CHANNELINDEX chn = 0; chn < m_nChannels; chn++)
			{
				if (file.Seek(expData.trackInfoOffset + chn * 4) && file.Seek(file.ReadUint32BE()))
				{
		...
		for (PATTERNINDEX pat = 0; pat < numPatterns; pat++)
		{
			if (!(loadFlags & loadPatternData) || !file.Seek(fileHeader.blockArrOffset + pat * 4u) || !file.Seek(file.ReadUint32BE()))
			{
				continue;
			}
			...
			else
			{
				...
				if (patHeader.blockInfoOffset)
				{
					...
					if (blockInfo.cmdExtTableOffset && file.Seek(blockInfo.cmdExtTableOffset) && file.Seek(file.ReadUint32BE()))
					{
